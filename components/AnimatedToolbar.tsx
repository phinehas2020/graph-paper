...
export function AnimatedToolbar({
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (isDragging && scrollRef.current) {
      ...
      const newIndex = Math.max(0, Math.min(tools.length - 1, Math.floor(relativeY / toolHeight)))
  ...
  return (
    <Card className={`shadow-xl border-0 bg-white/95 backdrop-blur-sm transition-all duration-300 ${isAnimating ? "scale-105" : ""} ${className}`}>
      <CardContent className={isMobile ? "p-1" : "p-2"}>
        <div
          ref={scrollRef}
          className={`flex ${orientation === "vertical" ? "flex-col" : "flex-row"} items-center gap-1 ${
            isMobile && orientation === "horizontal" ? "flex-wrap justify-center" : ""
          } overflow-auto scrollbar-hide`}
          onScroll={handleScroll}
          style={{ 
            maxHeight: orientation === "vertical" ? isMobile ? "60vh" : "50vh" : undefined,
            maxWidth: orientation === "horizontal" ? isMobile ? "90vw" : "70vw" : undefined
          }}
        >
          {tools.map((tool, index) => {
            ...
            return (
              <TooltipProvider key={tool.name} delayDuration={isMobile ? 0 : 100}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      `}
                      style={{
                        backgroundColor: isActive && tool.color ? `${tool.color}20` : undefined,
                        borderColor: isActive && tool.color ? tool.color : undefined,
                    ...
                    >
                      <Icon 
                        ...
                        style={{ color: isActive && tool.color ? tool.color : undefined }}
                      />
                  ...
                  <TooltipContent 
                    side={orientation === "vertical" ? "right" : isMobile ? "top" : "bottom"} 
