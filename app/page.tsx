...
const DEFAULT_WALL_HEIGHT = 8; // 8 feet
...
// Helper function to find walls connected to a specific wall's endpoints
function getConnectionsForWall(targetWall: Wall, allWalls: Wall[]): { startConnections: Wall[], endConnections: Wall[] } {
  const startConnections: Wall[] = [];
  const endConnections: Wall[] = [];

  if (!targetWall) return { startConnections, endConnections };

  for (const otherWall of allWalls) {
    if (otherWall.id === targetWall.id) continue;

    // Check connections to targetWall.start
    if (dist(targetWall.start, otherWall.start) < EPSILON || dist(targetWall.start, otherWall.end) < EPSILON) {
      if (!startConnections.find(w => w.id === otherWall.id)) { // Avoid duplicates if otherWall connects at both ends (unlikely for distinct walls)
        startConnections.push(otherWall);
      }
    }
    // Check connections to targetWall.end
    if (dist(targetWall.end, otherWall.start) < EPSILON || dist(targetWall.end, otherWall.end) < EPSILON) {
      if (!endConnections.find(w => w.id === otherWall.id)) {
        endConnections.push(otherWall);
      }
    }
  }
  // A wall found connected to start might also be connected to end.
  // The current structure separates them, which is fine.
  // If a wall is connected to both start and end of targetWall (e.g. forming a tiny U), it will appear in both lists.
  return { startConnections, endConnections };
}

// Helper function to find all walls connected at a specific point
function getConnectionsAtPoint(point: Point, allWalls: Wall[], excludingWallId?: string): Wall[] {
  const connectedWalls: Wall[] = [];
  for (const wall of allWalls) {
    if (excludingWallId && wall.id === excludingWallId) continue;

    if (dist(point, wall.start) < EPSILON || dist(point, wall.end) < EPSILON) {
...
export default function EnhancedGraphPaper() {
    ...
    (ctx: CanvasRenderingContext2D) => {
      ...
      currentState.lines.forEach((line) => {
        ...
        ctx.stroke()
          ...
          // Stud count display for Lines is removed as it's now part of Walls
      })
      ...
      // Draw Walls
      currentState.walls.forEach((wall) => {
        const isSelected = wall.id === selectedWallId;
        ctx.strokeStyle = isSelected ? "#3b82f6" : (wall.color || currentColor); // Blue if selected
        // Use wall.thickness, but maybe increase slightly if selected for better visibility
        const visualThickness = isSelected ? wall.thickness + 2/zoom : wall.thickness; // Add 2 pixels to thickness if selected (adjust by zoom)
        ctx.lineWidth = visualThickness * zoom;

        ctx.beginPath();
        ctx.moveTo(wall.start.x * zoom + panOffset.x, wall.start.y * zoom + panOffset.y);
        ctx.lineTo(wall.end.x * zoom + panOffset.x, wall.end.y * zoom + panOffset.y);
        ctx.stroke();

        // Optionally, draw small circles at endpoints if selected
        if (isSelected) {
            ctx.fillStyle = "#3b82f6";
            [wall.start, wall.end].forEach(p => {
                ctx.beginPath();
                ctx.arc(p.x * zoom + panOffset.x, p.y * zoom + panOffset.y, (visualThickness * zoom / 2) + 3, 0, 2 * Math.PI);
                ctx.fill();
            });
        }

        // Display stud count for walls
        if (wall.studCount !== undefined && wall.studCount > 0 && zoom > 0.2) {
          const midXWorld = (wall.start.x + wall.end.x) / 2;
          const midYWorld = (wall.start.y + wall.end.y) / 2;
          const midXCanvas = midXWorld * zoom + panOffset.x;
          const midYCanvas = midYWorld * zoom + panOffset.y;
          let textOffsetY = -Math.max(8, 10 * zoom);
          ...
          const text = `S: ${wall.studCount}`;
          ctx.strokeStyle = "white";
          ctx.lineWidth = 2 * zoom; // Text outline width
  ...
  const handlePointerDown = (point: Point, isShift: boolean, isTouchEvent = false) => {
    ...
    if (tool === "select" && !isPanning) {
        const clickWorldPoint = worldPoint; // Use the already calculated worldPoint
        let wallHit = false;
        if (designMode === "residential") {
            for (const wall of currentState.walls) {
                const selectionTolerance = Math.max(5, eraserWidth / 2);
                const worldSelectionTolerance = selectionTolerance / zoom;

                if (distancePointToLineSegment(clickWorldPoint, wall.start, wall.end) < worldSelectionTolerance) {
        ...
        // In select mode, if we're in residential, we've handled selection or deselection.
        // We don't want to proceed to other tool logic like starting shapes.
        if (designMode === "residential") return;
        // If not in residential design mode, select tool might do other things later,
        // but for now, wall selection is primary for residential.
    }
          ...
          switch (tool) {
            case "line":
              if (designMode === "residential") {
                ...
                const lengthInGridCells = dist(newWall.start, newWall.end) / gridSize;
                const activeStudSpacing = currentStudSpacing > 0 ? currentStudSpacing : 1.33;
                if (lengthInGridCells > EPSILON) {
                  newWall.studCount = Math.ceil(lengthInGridCells / activeStudSpacing) + 1;
                } else {
                  newWall.studCount = 0;
                }
                addToHistory({ ...currentState, walls: [...currentState.walls, newWall] });
              } else {
                addToHistory({ ...currentState, lines: [...currentState.lines, shapeData], walls: [...currentState.walls] });
              }
              break
            case "rectangle":
              const newRectangle = shapeData as Rectangle;
              let newLinesForTrusses: Line[] = [];
              const newWallsFromRect: Wall[] = [];
              ...
              if (designMode === "residential") {
                ...
                const spacing = currentTrussSpacing > 0 ? currentTrussSpacing : 2;
                ...
                // Create walls from rectangle sides
                const { start: rStart, end: rEnd, color: rectColor } = newRectangle;
                ...
                const rectSidesAsPoints: { p1: Point; p2: Point }[] = [
                ...
                const activeStudSpacingRect = currentStudSpacing > 0 ? currentStudSpacing : 1.33;
                rectSidesAsPoints.forEach(side => {
                  const wallId = crypto.randomUUID();
                  const wall: Wall = {
                    id: wallId,
                    start: side.p1,
                  ...
                  const lengthInGridCells = dist(wall.start, wall.end) / gridSize;
                  if (lengthInGridCells > EPSILON) {
                    wall.studCount = Math.ceil(lengthInGridCells / activeStudSpacingRect) + 1;
                  } else {
                    wall.studCount = 0;
                  }
                  newWallsFromRect.push(wall);
                });
              }
  ...
  const handlePointerUp = () => {
    if (isErasing && tool === "eraser" && eraserStrokePoints.length > 0) {
      ...
      let newWalls = [...currentState.walls]; // Initialize newWalls
      ...
      if (eraserMode === "partial") {
        ...
        originalLinesSnapshot.forEach((originalLine) => {
          // In residential mode, stud walls are handled by Wall eraser logic.
          // So, if it's a line with studCount (legacy or error), or if we are in graph mode, proceed.
          // For this refactor, lines with studCount should not exist if designMode is residential.
          // This partial erase logic now only applies to Lines (graph mode, or non-stud lines if any in residential)
          if (designMode === "residential" && originalLine.studCount !== undefined) {
             // This case should ideally not happen if lines are correctly converted to walls.
             // If it does, treat as whole erase for this line.
      ...
      // --- Wall Erasing (Residential Mode Only) ---
      if (designMode === "residential") {
        const originalWallCount = newWalls.length;
        newWalls = newWalls.filter(wall => {
            const isHit = eraserStrokePoints.some(ep => distancePointToLineSegment(ep, wall.start, wall.end) < eraserWidth / 2);
            if (isHit) { // In residential mode, any hit wall is removed entirely.
                return false;
            }
            return true;
        });
        if (newWalls.length !== originalWallCount) opPerformed = true;
      }
      ...
      if (designMode === "residential" && removedRectangles.length > 0) {
        ...
        removedRectangles.forEach(rect => {
          ...
          newLines.forEach(line => {
            if (line.thickness === 1) {
              const isHorizTruss = Math.abs(line.start.y - line.end.y) < EPSILON &&
                                   line.thickness === 1 &&
                                   (Math.abs(line.start.x - minX) < EPSILON && Math.abs(line.end.x - maxX) < EPSILON ||
                                    Math.abs(line.start.x - maxX) < EPSILON && Math.abs(line.end.x - minX) < EPSILON) &&
                                   line.start.y >= minY - EPSILON && line.start.y <= maxY + EPSILON;
              const isVertTruss =  Math.abs(line.start.x - line.end.x) < EPSILON &&
                                   line.thickness === 1 &&
                                   (Math.abs(line.start.y - minY) < EPSILON && Math.abs(line.end.y - maxY) < EPSILON ||
                                    Math.abs(line.start.y - maxY) < EPSILON && Math.abs(line.end.y - minY) < EPSILON) &&
                                   line.start.x >= minX - EPSILON && line.start.x <= maxX + EPSILON;
      ...
      if (opPerformed) {
        addToHistory({
          lines: newLines,
          walls: newWalls, // Add newWalls to history
          circles: newCircles,
          arcs: newArcs,
    ...
    try {
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Ensure that 'walls' property exists in the loaded history items
          const historyWithWalls = parsed.map(item => ({
            ...{ lines: [], arcs: [], rectangles: [], circles: [], arrows: [], texts: [], walls: [] }, // Default structure
            ...item, // Spread loaded item, potentially overwriting defaults
          }));
          setHistory(historyWithWalls);
          setHistoryIndex(historyWithWalls.length - 1);
        }
      }
    } catch (err) {
  ...
  return (
    <div className="w-screen h-screen overflow-hidden relative bg-gradient-to-br from-slate-50 to-slate-100 touch-none">
              ...
              <Button
                ...
                onClick={() => {
                  setDesignMode(prevMode => {
                    setSelectedWallId(null); // Deselect wall
                    return prevMode === "graph" ? "residential" : "graph";
                  });
                  triggerFeedback();
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        <Undo className="w-5 h-5 text-slate-600 text-slate-600 text-black text-black bg-black bg-black bg-white bg-black bg-black" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>Undo (Ctrl+Z)</p>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        disabled={tool !== "eraser"}
                      >
                        {eraserMode === "partial" ? <Scissors className="w-5 h-5" /> : <Trash2 className="w-5 h-5" />}
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>Eraser: {eraserMode === "partial" ? "Partial (Lines Only)" : "Whole Element"}</p>
      ...
      {/* Wall Info Panel */}
      {selectedWallId && designMode === "residential" && (() => {
        const selectedWall = currentState.walls.find(w => w.id === selectedWallId);
        if (!selectedWall) return null;

        const wallLength = dist(selectedWall.start, selectedWall.end) / gridSize; // In grid units

        return (
          <div className="absolute bottom-6 right-6 z-20 transition-all duration-300">
            <Card className="shadow-xl bg-white/95 backdrop-blur-sm">
              <CardContent className="p-3">
                <h3 className="text-sm font-semibold mb-2">Wall Details</h3>
                <div className="text-xs space-y-1">
                  <p>ID: <span className="font-mono text-gray-600">{selectedWall.id.substring(0,8)}...</span></p>
                  <p>Length: {wallLength.toFixed(2)} ft (approx)</p> {/* Assuming grid unit = 1ft */}
                  <p>Thickness: {selectedWall.thickness.toFixed(2)} ft</p>
                  <p>Height: {selectedWall.height.toFixed(2)} ft</p>
                  <p>Studs: {selectedWall.studCount || 0}</p>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                  className="w-full mt-3"
                  onClick={() => { /* alert('Edit Wall Elevation - Not implemented yet'); */ }}
                  // disabled // Keep it enabled to show it's a future feature
                >
                  Edit Wall Elevation (Soon)
                </Button>
              </CardContent>
            </Card>
          </div>
        ...
        if (dist(activeShapeStartPoint, snappedPoint) > EPSILON) {
          ...
          switch (tool) {
            case "line":
              if (designMode === "residential") {
                ...
                const newWall: Wall = {
                  ...
                  thickness: currentThickness, // Use global currentThickness for now
                  height: DEFAULT_WALL_HEIGHT,
                  color: shapeData.color, // from currentColor
                  elevationData: {}, // Placeholder
                };
                // Stud calculation logic
                const lengthInGridCells = dist(newWall.start, newWall.end) / gridSize;
                const activeStudSpacing = currentStudSpacing > 0 ? currentStudSpacing : 1.33;
                if (lengthInGridCells > EPSILON) {
                  newWall.studCount = Math.ceil(lengthInGridCells / activeStudSpacing) + 1;
                ...
                addToHistory({ walls: [...currentState.walls, newWall] });
              } else { // Original behavior for "graph" mode
            ...
            case "rectangle":
              ...
              // Create walls from rectangle sides if in residential mode
              const newWallsFromRect: Wall[] = [];
              if (designMode === "residential") {
                const { start: rStart, end: rEnd, color: rectColor } = newRectangle;
                const minX = Math.min(rStart.x, rEnd.x);
                ...
                rectSidesAsPoints.forEach(side => {
                  ...
                  const wall: Wall = {
                    ...
                    thickness: currentThickness, // Using global currentThickness for walls from rect
                    height: DEFAULT_WALL_HEIGHT,
                    color: rectColor,
                    elevationData: {},
                  };
