...
import { buildFloorLayoutPreview } from '@pascal-app/construction'
import {
  type AnyNodeId,
  type BuildingNode,
...
const FLOORPLAN_FLOOR_LAYOUT_STROKE_WIDTH = 0.035
const FLOORPLAN_FLOOR_LAYOUT_SUPPORT_STROKE_WIDTH = 0.055
const FLOORPLAN_FLOOR_LAYOUT_ARROW_SIZE = 0.22
...
function getFloorDirectionArrow(
  axis: 'x' | 'z',
  centroid: Point2D,
  ...
  const lineLength = Math.max(
    0.8,
    Math.min(
      axis === 'x' ? bounds.width : bounds.depth,
      axis === 'x' ? bounds.depth : bounds.width,
    ) * 0.45,
  )
  const halfLength = lineLength / 2
  const arrowSize = Math.min(FLOORPLAN_FLOOR_LAYOUT_ARROW_SIZE, lineLength * 0.18)
  const start =
    axis === 'x'
      ? { x: centroid.x - halfLength, y: centroid.y }
      : { x: centroid.x, y: centroid.y - halfLength }
  const end =
    axis === 'x'
      ? { x: centroid.x + halfLength, y: centroid.y }
      : { x: centroid.x, y: centroid.y + halfLength }
  ...
  const arrowHead =
    axis === 'x'
      ? [
          { x: end.x - arrowSize, y: end.y - arrowSize * 0.7 },
          { x: end.x, y: end.y },
          { x: end.x - arrowSize, y: end.y + arrowSize * 0.7 },
        ]
      : [
          { x: end.x - arrowSize * 0.7, y: end.y - arrowSize },
          { x: end.x, y: end.y },
          { x: end.x + arrowSize * 0.7, y: end.y - arrowSize },
        ]

  return { start, end, arrowHead }
}
...
const FloorplanGeometryLayer = memo(function FloorplanGeometryLayer({
  ...
  onSlabJoistDirectionCycle: (slab: SlabNode, event: ReactMouseEvent<SVGElement>) => void
  ...
  return (
    <>
      {slabPolygons.map(({ slab, polygon, holes, path }) => {
        ...
        const isFloorLayoutVisible = isSelected && slab.framingStrategy !== 'slab-on-grade'
        const layoutPreview = isFloorLayoutVisible
          ? buildFloorLayoutPreview({
              polygon: slab.polygon,
              holes: slab.holes ?? [],
              joistDirection: slab.joistDirection,
              joistSpacing: slab.joistSpacing,
        ...
        if (isSelected) {
          ...
          if (layoutPreview) {
            const arrow = getFloorDirectionArrow(layoutPreview.axis, centroid, layoutPreview.bounds)
            const directionLabel =
              slab.joistDirection === 'auto' ? 'A' : layoutPreview.axis.toUpperCase()

            floorLayout = (
              <g>
                {layoutPreview.joistSegments.map((segment, index) => (
                  <line
                    key={`joist-${index}`}
                    pointerEvents="none"
                    stroke={palette.measurementStroke}
                    strokeOpacity={0.38}
                    strokeWidth={FLOORPLAN_FLOOR_LAYOUT_STROKE_WIDTH}
                    vectorEffect="non-scaling-stroke"
                ...
                {layoutPreview.supportSegments.map((segment, index) => (
                  <line
                    key={`support-${index}`}
                    pointerEvents="none"
                    stroke={palette.selectedStroke}
                    strokeOpacity={0.62}
                    strokeWidth={FLOORPLAN_FLOOR_LAYOUT_SUPPORT_STROKE_WIDTH}
                    vectorEffect="non-scaling-stroke"
                    x1={toSvgX(segment.start[0])}
                    x2={toSvgX(segment.end[0])}
                    y1={toSvgY(segment.start[1])}
                ...
                <line
                  ...
                  strokeWidth={FLOORPLAN_FLOOR_LAYOUT_SUPPORT_STROKE_WIDTH}
                  vectorEffect="non-scaling-stroke"
                  x1={toSvgX(arrow.start.x)}
                  x2={toSvgX(arrow.end.x)}
                  y1={toSvgY(arrow.start.y)}
                  y2={toSvgY(arrow.end.y)}
                />
                <polyline
                  fill="none"
                  pointerEvents="none"
                ...
                {canSelectSlabs ? (
                  <g
                    onClick={(event) => {
                      event.stopPropagation()
                      onSlabJoistDirectionCycle(slab, event)
                    }}
                    style={{ cursor: EDITOR_CURSOR }}
                  >
                    <circle
                      cx={toSvgX(arrow.start.x)}
                      cy={toSvgY(arrow.start.y)}
                    ...
                    <text
                      ...
                      fontWeight="700"
                      textAnchor="middle"
                      x={toSvgX(arrow.start.x)}
                      y={toSvgY(arrow.start.y)}
                    >
                      {directionLabel}
                    </text>
                  </g>
...
export function FloorplanPanel() {
  ...
  const handleSlabJoistDirectionCycle = useCallback(
    (slab: SlabNode, event: ReactMouseEvent<SVGElement>) => {
      event.stopPropagation()
      const nextDirection =
        slab.joistDirection === 'auto' ? 'x' : slab.joistDirection === 'x' ? 'z' : 'auto'
      updateNode(slab.id, {
        joistDirection: nextDirection,
      })
    },
    [updateNode],
  )
        ...
        {!levelNode || levelNode.type !== 'level' ? (
          ...
          >
            ...
            <FloorplanGeometryLayer
              ...
              onSlabJoistDirectionCycle={handleSlabJoistDirectionCycle}
