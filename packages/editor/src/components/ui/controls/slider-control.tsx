...
export function SliderControl({
  ...
  const isLengthUnit = unit === 'm'
  const formatDisplayValue = useCallback(
    (nextValue: number) =>
      isLengthUnit ? formatLengthImperial(nextValue) : nextValue.toFixed(precision),
    [isLengthUnit, precision],
  )
  ...
  }, [value, formatDisplayValue, isEditing])
  ...
  const submitValue = useCallback(() => {
    const numValue = isLengthUnit ? parseLengthToMeters(inputValue) : Number.parseFloat(inputValue)
    ...
    setIsEditing(false)
  }, [inputValue, isLengthUnit, onChange, clamp, precision, value, formatDisplayValue])
  ...
  const handleInputKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === 'Enter') {
        ...
        onChange(newV)
        setInputValue(formatDisplayValue(newV))
    ...
    [submitValue, value, formatDisplayValue, step, clamp, onChange],
  )
  ...
  return (
    ...
    >
      ...
      <div className={cn('flex shrink-0 justify-end', isLengthUnit ? 'w-[104px]' : 'w-[50px]')}>
        {isEditing ? (
          <div className="flex items-center">
            <input
              ...
              value={inputValue}
            />
            {!isLengthUnit && unit && <span className="ml-[1px] text-muted-foreground">{unit}</span>}
          </div>
        ) : (
          <div
          ...
          >
            <span className="font-mono tabular-nums tracking-tight">
              {isLengthUnit ? formatLengthImperial(value) : Number(value.toFixed(precision)).toFixed(precision)}
            </span>
            {!isLengthUnit && unit && <span className="ml-[1px] text-muted-foreground">{unit}</span>}
          </div>
        )}
