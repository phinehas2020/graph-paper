...
function ImperialLengthInput({
  ...
  const [draft, setDraft] = useState(() => formatLengthImperial(value))

  useEffect(() => {
    setDraft(formatLengthImperial(value))
  }, [value])

  const commit = () => {
    const parsed = parseLengthToMeters(draft)
    ...
    onChange(parsed)
    setDraft(formatLengthImperial(parsed))
  }

  return (
    <input
      className="w-20 rounded border border-border/50 bg-accent/50 px-1.5 py-0.5 text-foreground text-xs focus:border-primary focus:outline-none"
      onBlur={commit}
    ...
    <div className="relative border-border/50 border-b">
      ...
      <div className="relative flex gap-3 pr-3 pb-2 pl-10">
        <div className="text-muted-foreground text-xs">
          Area: <span className="text-foreground">{formatAreaImperial(area, 1)}</span>
        </div>
        <div className="text-muted-foreground text-xs">
          Perimeter: <span className="text-foreground">{formatLengthImperial(perimeter)}</span>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-1">
            {points.map((point, index) => (
              <div className="flex items-center gap-1.5 text-xs" key={index}>
                ...
                <ImperialLengthInput
                  onChange={(nextValue) => handlePointChange(index, 0, nextValue)}
                  value={point[0]}
                />
                <label className="shrink-0 text-muted-foreground">Z</label>
                <ImperialLengthInput
                  onChange={(nextValue) => handlePointChange(index, 1, nextValue)}
                  value={point[1]}
                />
                <button
                  className={cn(
...
function ZoneItem({ zone, isLast }: { zone: ZoneNode; isLast?: boolean }) {
  ...
  const defaultName = `Zone (${formatAreaImperial(area, 1)})`
