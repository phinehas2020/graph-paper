    ...
    >
      <PanelSection title="Elevation">
        ...
        <div className="mt-2 grid grid-cols-2 gap-1.5 px-1 pb-1">
          <ActionButton
            label={`Sunken (${formatLengthImperial(-0.15)})`}
            onClick={() => handleUpdate({ elevation: -0.15 })}
          />
          <ActionButton
            label={`Ground (${formatLengthImperial(0)})`}
            onClick={() => handleUpdate({ elevation: 0 })}
          />
          <ActionButton
            label={`Raised (${formatLengthImperial(0.05)})`}
            onClick={() => handleUpdate({ elevation: 0.05 })}
          />
          <ActionButton
            label={`Step (${formatLengthImperial(0.15)})`}
            onClick={() => handleUpdate({ elevation: 0.15 })}
          />
        </div>
      </PanelSection>
      ...
      <PanelSection title="Info">
        <div className="flex items-center justify-between px-2 py-1 text-muted-foreground text-sm">
          <span>Area</span>
          <span className="font-mono text-white">{formatAreaImperial(area, 2)}</span>
        </div>
      </PanelSection>
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {formatAreaImperial(holeArea, 2)} · {hole.length} pts
                    </p>
                  </div>
