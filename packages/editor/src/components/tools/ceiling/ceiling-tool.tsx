...
export const CeilingTool: React.FC = () => {
  ...
  const setPointsAndUpdateRef = useCallback((nextPoints: Array<[number, number]>) => {
  ...
  const openLengthInput = useCallback(() => {
    ...
    const snapPoint = snappedCursorRef.current
    const dx = snapPoint[0] - lastPoint[0]
    const dz = snapPoint[1] - lastPoint[1]
    const currentLength = Math.sqrt(dx * dx + dz * dz)
    if (!Number.isFinite(currentLength) || currentLength === 0) return

    setLengthInputValueState(formatLength(currentLength))
    setLengthInputPosition([snapPoint[0], levelYRef.current + GRID_OFFSET, snapPoint[1]])
  ...
  const submitLength = useCallback(() => {
    ...
    const snapPoint = snappedCursorRef.current
    const projected = projectPointAlongDirection(lastPoint, snapPoint, requestedLength)
    if (!projected) return
  ...
  useEffect(() => {
    ...
    const onGridMove = (event: GridEvent) => {
      ...
      snappedCursorRef.current = displayPoint
    ...
    const onGridClick = (_event: GridEvent) => {
      ...
      if (
        ...
        const ceilingId = commitCeilingDrawing(currentLevelId, currentPoints)
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Tab' && pointsRef.current.length > 0) {
        e.preventDefault()
