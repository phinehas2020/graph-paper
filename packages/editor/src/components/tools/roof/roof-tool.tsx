...
export const RoofTool: React.FC = () => {
  ...
  const openLengthInput = useCallback(() => {
    ...
    const cursor = snappedCursorRef.current
    const dx = cursor[0] - corner1[0]
    const dz = cursor[1] - corner1[2]
    const currentLength = Math.sqrt(dx * dx + dz * dz)
    if (!Number.isFinite(currentLength) || currentLength === 0) return

    setLengthInputValueState(formatLength(currentLength))
    setLengthInputPosition([cursor[0], levelYRef.current + GRID_OFFSET, cursor[1]])
  ...
  const submitLength = useCallback(() => {
    ...
    const roofId = commitRoofPlacement(currentLevelId, corner1, [
      projected[0],
      corner1[1],
      projected[1],
    ])
    ...
    closeLengthInput()
  }, [closeLengthInput, currentLevelId, setSelection])
  ...
  useEffect(() => {
    ...
    const onGridMove = (event: GridEvent) => {
      ...
      const snappedCursor: [number, number] = [gridX, gridZ]
      ...
      if (isLengthInputOpenRef.current) {
        setLengthInputPosition([gridX, gridY, gridZ])
      }
    ...
    const onGridClick = (event: GridEvent) => {
      ...
      if (corner1Ref.current) {
        ...
        levelYRef.current = y
        setLengthInputPosition([gridX, y + GRID_OFFSET, gridZ])
      }
    }
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Tab' && corner1Ref.current) {
  ...
  }, [closeLengthInput, currentLevelId, openLengthInput, setSelection])
