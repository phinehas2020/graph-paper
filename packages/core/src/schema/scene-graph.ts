...
import { AnyNode, type AnyNode as AnyNodeType, type AnyNodeId } from './types'
...
function isNodeLike(value: unknown): value is Record<string, unknown> & { id: string; type: string } {
  return Boolean(
    value &&
      typeof value === 'object' &&
...
function normalizeSceneNode(
  value: unknown,
  normalizedNodes: Record<string, AnyNodeType>,
  forcedParentId: string | null = null,
): AnyNodeType | null {
  if (!isNodeLike(value)) {
    return null
  }
...
function migrateLegacyNodes(nodes: Record<string, AnyNodeType>): Record<AnyNodeId, AnyNodeType> {
  const patchedNodes = { ...nodes } as Record<string, AnyNodeType>
  ...
  for (const [id, node] of Object.entries(patchedNodes)) {
    ...
    if (node.type === 'roof' && !('children' in node)) {
      const oldRoof = node as Record<string, unknown>
      ...
      const segment = AnyNode.parse({
        ...
        depth:
          (typeof oldRoof.leftWidth === 'number' ? oldRoof.leftWidth : 2.2) +
  ...
  return patchedNodes as Record<AnyNodeId, AnyNodeType>
}

function normalizeRootNodeIds(
  inputRootNodeIds: unknown,
  inputNodes: Record<string, unknown>,
  normalizedNodes: Record<string, AnyNodeType>,
): AnyNodeId[] {
  ...
  return Array.from(normalizedRootIds) as AnyNodeId[]
}

function ensureSiteRoots(
  nodes: Record<AnyNodeId, AnyNodeType>,
  rootNodeIds: AnyNodeId[],
): { nodes: Record<AnyNodeId, AnyNodeType>; rootNodeIds: AnyNodeId[] } {
  ...
  const otherRootIds = rootNodeIds.filter(
    (rootId) => !siteRootIds.includes(rootId) && !siteEligibleRootIds.includes(rootId),
  )

  if (siteEligibleRootIds.length === 0) {
    return {
      nodes: nextNodes,
      rootNodeIds: Array.from(new Set([...siteRootIds, ...otherRootIds])) as AnyNodeId[],
    }
  }
  ...
  if (siteRootIds.length > 0) {
    ...
    if (primarySite?.type === 'site') {
      ...
      for (const rootId of siteEligibleRootIds) {
        childIds.add(rootId as `building_${string}` | `item_${string}`)
  ...
  const syntheticSite = SiteNode.parse({
    children: siteEligibleRootIds as Array<`building_${string}` | `item_${string}`>,
  })
  ...
  return {
    nodes: nextNodes,
    rootNodeIds: [syntheticSite.id, ...otherRootIds] as AnyNodeId[],
  }
}
...
export function createDefaultSceneGraph(): NormalizedSceneGraph {
...
export function createSceneGraphSnapshot(
  nodes: Record<AnyNodeId, AnyNodeType>,
  rootNodeIds: AnyNodeId[],
  sceneSchemaVersion = SCENE_SCHEMA_VERSION,
...
export function migrateSceneGraph(sceneGraph: SceneGraph | null | undefined): NormalizedSceneGraph {
  if (!(sceneGraph && isSceneGraph(sceneGraph))) {
    return createDefaultSceneGraph()
  }

  const normalizedNodes: Record<string, AnyNodeType> = {}
  const rootNodeIds = normalizeRootNodeIds(
    sceneGraph.rootNodeIds,
    sceneGraph.nodes,
    normalizedNodes,
  )
