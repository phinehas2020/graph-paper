...
function buildRoofPlaneResult(roofPlane, rulePack) {
    ...
    if (centroid && roofPlane.area > 0) {
        members.push(createSurfaceMember({
...
function buildRoofArrayResult(member, roofPlaneById, rulePack) {
    ...
    if (member.length <= 0) {
        ...
        if (framingWidth <= 0 || framingRun <= 0) {
            ...
            for (let index = 0; index < count; index += 1) {
                ...
                const startPlan = fromBasis(bounds.minAlongDirection, offset, bounds.direction, bounds.normal);
                const endPlan = fromBasis(bounds.maxAlongDirection, offset, bounds.direction, bounds.normal);
...
export function compileRoofFraming(topology, rulePack) {
    const roofPlaneById = Object.fromEntries(topology.roofPlanes.map((roofPlane) => [roofPlane.roofPlaneId, roofPlane]));
    return [
        ...topology.roofPlanes.map((roofPlane) => buildRoofPlaneResult(roofPlane, rulePack)),
        ...topology.trussArrays.map((trussArray) => buildRoofArrayResult({ ...trussArray, sourceNodeType: 'truss-array' }, roofPlaneById, rulePack)),
        ...topology.rafterSets.map((rafterSet) => buildRoofArrayResult({ ...rafterSet, sourceNodeType: 'rafter-set' }, roofPlaneById, rulePack)),
    ];
}
