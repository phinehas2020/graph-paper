import { createMember, createPointMember, createSurfaceMember, finalizeComponentResult, polygonBounds, } from '../shared';
const SUBFLOOR_SHEET_AREA = 1.2192 * 2.4384;
export function compileFloorFraming(topology, rulePack) {
    const results = [];
    topology.floorSystems.forEach((system) => {
        ...
        const members = diagnostics.length > 0 ? [] : [
            createMember({
                ...
                category: 'framing',
                label: 'Floor Joists',
                materialCode: system.framingKind === 'dimensional-lumber'
                    ? 'floor.joist'
    ...
    topology.floorOpenings.forEach((opening) => {
        const diagnostics = [];
        const members = opening.curbHeight > 0 && opening.perimeter > 0
            ? [
                createSurfaceMember({
