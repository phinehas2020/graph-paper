function getFallbackWasteFactor(member, rulePack) {
    if (member.type === 'sheathing')
        return rulePack.wasteFactors.sheathing;
    if (member.type === 'drywall')
        return rulePack.wasteFactors.drywall;
    if (member.type === 'trim')
        return rulePack.wasteFactors.trim;
    return rulePack.wasteFactors.framing;
}
export function buildQuantityLines(members, rulePack, options = {}) {
    ...
    for (const member of members) {
        ...
        if (!existing) {
            lines.set(key, {
                ...
                estimatedTotalCost: costRule ? member.quantity * (1 + wasteFactor) * costRule.unitCost : undefined,
