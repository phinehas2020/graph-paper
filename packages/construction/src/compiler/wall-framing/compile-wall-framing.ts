...
import {
  type ConstructionMember,
  type ConstructionMemberCategory,
  type ConstructionMemberType,
  type ConstructionTopologyWall,
  type Vec3,
  type WallCompileResult,
  type WallOpening,
} from '../../schema/construction-graph'
import type { QuantityLine } from '../../schema/quantities'
import type { RulePack } from '../../schema/rulepacks'
import type { WallAssemblyDefinition } from '../../schema/assemblies'
...
function getWallBasis(wall: ConstructionTopologyWall): WallBasis {
...
function localToWorld(wall: ConstructionTopologyWall, basis: WallBasis, local: Vec3): Vec3 {
  return [
...
function openingBounds(opening: WallOpening) {
  return {
...
function getStudLayout(opening: WallOpening, assembly: WallAssemblyDefinition) {
  const studThickness = assembly.stud.actualThickness
  const { left, right } = openingBounds(opening)
  const leftJacks = Array.from(
    { length: assembly.openingFraming.jackStudsPerSide },
    (_, index) => left - studThickness / 2 - index * studThickness,
  )
  const rightJacks = Array.from(
    { length: assembly.openingFraming.jackStudsPerSide },
    (_, index) => right + studThickness / 2 + index * studThickness,
  )
  const leftKings = Array.from(
    { length: assembly.openingFraming.kingStudsPerSide },
    (_, index) => leftJacks[leftJacks.length - 1]! - studThickness * (index + 1),
  )
  const rightKings = Array.from(
    { length: assembly.openingFraming.kingStudsPerSide },
    (_, index) => rightJacks[rightJacks.length - 1]! + studThickness * (index + 1),
  )
...
function getCommonStudCenters(
  ...
  const reservedRanges = wall.openings.map((opening) => {
    ...
    return [layout.reservedStart, layout.reservedEnd] as const
  })
  const centers = new Set<number>()
  const addCenter = (center: number) => {
    const clamped = clamp(center, studThickness / 2, wall.length - studThickness / 2)
    const insideReservedRange = reservedRanges.some(
      ([min, max]) => clamped >= min - EPSILON && clamped <= max + EPSILON,
    )
...
function getSurfaceRects(wall: ConstructionTopologyWall): Rect[] {
  let rects: Rect[] = [
    {
      x: 0,
      y: 0,
...
function tileRect(rect: Rect, sheetWidth: number, sheetHeight: number): Rect[] {
  const tiles: Rect[] = []
...
function getFinishFaces(
  faceCount: number,
  wall: ConstructionTopologyWall,
  defaultSingleFace: 'interior' | 'exterior',
) {
  if (faceCount <= 0) return [] as const
  if (faceCount === 1) {
    return [wall.isExterior ? defaultSingleFace : 'interior'] as const
  }

  return ['interior', 'exterior'] as const
}

function addSurfaceMembers(
  members: ConstructionMember[],
  createMember: ReturnType<typeof createMemberFactory>,
  wall: ConstructionTopologyWall,
  assembly: WallAssemblyDefinition,
  type: 'sheathing' | 'drywall',
...
function addTrimMembers(
  ...
  if (!assembly.trim.enabled) return
  ...
  const verticalPieces: Array<{ x: number; y: number; length: number }> = [
    {
...
export function compileWallFraming(input: {
  ...
  for (const opening of wall.openings) {
    ...
    const headerLength =
      opening.width + assembly.openingFraming.headerBearingLength * 2
    ...
    const studGroups: Array<{
      type: 'king-stud' | 'jack-stud'
      centers: number[]
      top: number
      label: string
    }> = [
    ...
    for (
      let cursor = layout.left + crippleSpacing;
      cursor < layout.right - crippleSpacing / 2;
      cursor += crippleSpacing
    ) {
