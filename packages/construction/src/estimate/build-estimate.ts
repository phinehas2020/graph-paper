import type { ConstructionEstimate, EstimateLine, EstimateRollup } from '../schema/estimate'
import type { QuantityLine } from '../schema/quantities'

export function createEmptyEstimate(currency = 'USD'): ConstructionEstimate {
  return {
    lines: [],
    rollups: [],
    summary: {
...
export function buildEstimate(
  ...
  currency = quantities[0]?.currency ?? 'USD',
): ConstructionEstimate {
  const lines: EstimateLine[] = quantities
  ...
  const rollupMap = new Map<string, EstimateRollup>()
